...
enum class receive_guard_state : int64_t { idle, in_progress, recovery_required, retired };
...
struct receive_guard_snapshot {
    ...
    int64_t store_incarnation = 0, store_channels = 0, store_channel_bytes = 0;
    receive_guard_state state = receive_guard_state::idle;
    receive_guard_reason reason = receive_guard_reason::none;
    // Present + null is initialized beginning, never a latest-AuditLog fallback.
